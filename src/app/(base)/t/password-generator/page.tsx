...
export default function PasswordGeneratorPage() {
  ...
  const { options, length, toggleOption, setLength } =
    usePasswordGeneratorStore((state) => ({
  ...
  const generatePassword = useCallback(
    (options: { [key in Options]: boolean }, length: number) => {
  ...
  return (
    <Main>
      <Card>
        <CardBody
          className={
            "flex flex-col space-y-8 md:flex-row md:space-x-8 md:space-y-0"
          }
        >
          <div className={"flex-1 space-y-4"}>
            ...
            <div>
              <div
                className={clsx(
                  "mb-2 h-1 transition-size",
                  SCORE_STYLES[password.score].bar,
                )}
              />
              <div>
                <p className={"mr-1 inline"}>
                  <span
                    className={clsx(
                      ...
                      SCORE_STYLES[password.score].color,
                    )}
                  >
          ...
          <div className={"flex-1 space-y-4"}>
            ...
            <Button
              className={"w-full"}
              onClick={async () => await generatePassword(options, length)}
            >
              Generate
            </Button>
          </div>

...
import {
  ...
  SCALES,
  usePxConvertorStore,
} from "~/app/(base)/t/px-converter/data";
    ...
    <Main>
      <Card>
        <CardBody className={"space-y-8"}>
            ...
            <div>
                  ...
                  onChange={(event) => {
                    ...
                    if (!isNaN(baseAsNum) && !isNaN(leftAsNum)) {
                      setRight(
                        String(
                          isPxToRem
                            ? calculateRem(leftAsNum, baseAsNum)
                            : calculatePx(leftAsNum, baseAsNum),
                        ),
                      );
                    }
                  }}
              ...
              <Popover>
                ...
                <PopoverContent className={"max-w-xs p-2 text-sm"}>
                  <p>
                    Font size of the root element. Most browser defaults to
                    16px.
                  </p>
                </PopoverContent>
              </Popover>
          ...
          <div
            className={"flex flex-col items-center sm:flex-row sm:items-end"}
          >
                ...
                onChange={(event) => {
                  ...
                  if (!isNaN(rightAsNum)) {
                    setLeft(
                      String(
                        isPxToRem
                          ? calculatePx(rightAsNum, baseAsNum)
                          : calculateRem(rightAsNum, baseAsNum),
                      ),
                    );
                  }
                }}
        ...
        <CardBody>
          <div className={"-my-2 flex flex-wrap"}>
            {SCALES.map((bucket, rowIndex) => {
              return (
                <div
                  className={"my-2 w-full flex-1 basis-64 overflow-auto"}
                  key={`bucket-${rowIndex}`}
                >
                      ...
                      {bucket.map((value) => {
                        return (
                          <tr key={value}>
                            ...
                            <TableCell>
                              {parseFloat(
                                calculateRem(value, parseFloat(base)).toFixed(
                                  4,
                                ),
                              )}

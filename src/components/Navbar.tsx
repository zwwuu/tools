...
export default function Navbar() {
  ...
  return (
    <div
      className={"sticky top-0 z-100 flex items-center border-b bg-base-100"}
    >
        ...
        <NavigationMenu.List className={"flex space-x-2"}>
          <NavigationMenu.Item className={"md:hidden"}>
            <NavigationMenu.Trigger
              aria-label={"Menu"}
              className={
                "flex h-full items-center border-r p-4 transition hover:opacity-80 focus:opacity-80"
              }
            >
            ...
            <NavigationMenu.Content asChild>
              <div
                className={
                  "absolute top-full left-0 mt-4 border-md bg-base-300 data-[state=closed]:animate-fadeOut data-[state=closed]:animate-faster data-[state=open]:animate-fadeIn data-[state=open]:animate-faster"
                }
              >
                <ul>
          ...
          {navLinks.map(({ href, label, hideOnMobile }) => (
            <NavigationMenu.Item
              className={clsx(hideOnMobile && "hidden md:block")}
              key={href}
            >
              <NavLink active={pathname === href} href={href}>
                {label}
          ...
          <NavigationMenu.Indicator
            className={"group absolute top-full left-0 flex justify-center"}
            forceMount
          >
            <div
              className={
                "w-2 bg-black transition-size group-data-[state=hidden]:h-0 group-data-[state=visible]:h-4"
              }
            />
          </NavigationMenu.Indicator>
        </NavigationMenu.List>
